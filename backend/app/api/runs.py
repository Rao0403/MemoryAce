...
from ..schemas import RunEndCreate, RunEndRead, RunStartCreate, RunStartRead, TrialEventsBatchCreate, TrialEventsBatchRead
...
@router.post("/runs/start", response_model=RunStartRead)
...
@router.post("/runs/{run_id}/events/batch", response_model=TrialEventsBatchRead)
def add_run_events(
    run_id: int,
...
@router.post("/runs/{run_id}/end", response_model=RunEndRead)
def end_run(
    run_id: int,
    ...
    if normalized_reason not in VALID_RUN_END_REASONS:
        raise HTTPException(status_code=400, detail=f"Invalid end_reason: {payload.end_reason}")
